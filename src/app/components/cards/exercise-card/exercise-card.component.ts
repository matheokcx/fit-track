...
} from '../../../../pipes/exercise-progression-manage/exercise-progression-manage.pipe';
...
export class ExerciseCardComponent implements OnInit {
    ...
    private async getAdvancementsFromlastTime(): Promise<number> {
        ...
        if (lastBeforeWorkout === null) {
            return 0;
        }

        const lastTimeExercise: FinishedExercise | undefined =
            lastBeforeWorkout?.finishedExercise.find(
                (exercise: FinishedExercise) =>
                    exercise.exercise.name === this.exercise.exercise.name,
            );
