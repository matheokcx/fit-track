import {Component} from '@angular/core';
import {ExerciseFullInfosComponent} from '../exercise-full-infos/exercise-full-infos.component';
import {IonSearchbar, IonSelect, IonSelectOption,} from '@ionic/angular/standalone';
import {Exercise, EXERCISES, musclesList} from '../../../models/exercise';
import {FormsModule} from '@angular/forms';

@Component({
    selector: 'app-exercises-segment-view',
    templateUrl: './exercises-segment-view.component.html',
    styleUrls: ['./exercises-segment-view.component.scss'],
    imports: [
        ExerciseFullInfosComponent,
        IonSearchbar,
        IonSelect,
        IonSelectOption,
        FormsModule
    ]
})
export class ExercisesSegmentViewComponent {
    protected allExercises: Exercise[] = EXERCISES;
    protected readonly musclesList: string[] = musclesList;
    protected muscleFilter: string | null = null;

    protected handleSearchInput(event: Event): void {
        const element = event.target as HTMLInputElement;
        const searchPart: string = element.value;
        this.allExercises = searchPart !== '' ? EXERCISES.filter(
            (exercise: Exercise) => exercise.name.includes(searchPart)
        ) : EXERCISES;
    }

    protected filterExercisesByMuscle(): void {
        if (this.muscleFilter !== 'Tous') {
            this.allExercises = EXERCISES.filter(
                (exercise: Exercise) => exercise.usedMuscles.includes(this.muscleFilter ?? ''),
            );
        }
        else {
            this.allExercises = EXERCISES;
        }
    }
}
